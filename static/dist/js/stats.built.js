  ...
  if (Store.get('showPokemon')) {
    ...
    for (i = 0; i < pkmnCount.length; i++) {
      if (pkmnCount[i] && pkmnCount[i].Count > 0) {
        pokeCounts.push(['<img src=\'static/icons/' + pkmnCount[i].ID + '.png\' />', '<a href=\'http://www.pokemon.com/us/pokedex/' + pkmnCount[i].ID + '\' target=\'_blank\' title=\'View in Pokédex\' style=\'color: black;\'>' + pkmnCount[i].Name + '</a>', pkmnCount[i].Count, Math.round(pkmnCount[i].Count * 100 / pkmnTotal * 10) / 10 + '%']);
      }
    }
